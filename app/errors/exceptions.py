...
class NoRequiredArgument(APIException):
    def __init__(self, ex: Exception = None):
        super().__init__(
            status_code=StatusCode.HTTP_406,
            msg=f"필수 인자가 누락되었습니다.",
            detail="RequestParameter Required",
            code=f"{StatusCode.HTTP_406}{'1'.zfill(4)}",
            ex=ex,
        )
...
class NotSupported(APIException):
    def __init__(self, ex: Exception = None):
        super().__init__(
            status_code=StatusCode.HTTP_407,
            msg=f"지원하지 않는 기능 입니다.",
            detail="Not Supported",
            code=f"{StatusCode.HTTP_407}{'1'.zfill(4)}",
            ex=ex,
        )
